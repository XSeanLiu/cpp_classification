...
int main(int argc, char** argv) {
  ...
  Classifier classifier(model_file, trained_file, mean_file, label_file, device_id);


  ifstream in(filelist.c_str());
  string line;
  CHECK(in) << "Unable to open the file ";
  int label;
  int pred_label;
  string imgname;
  int succ(0), total(0);
  ...
  LOG(ERROR)<<"Precision : "<<100.0*float(succ)/float(total)<<"%";
#if 0
